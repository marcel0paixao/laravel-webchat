...
export default function UsersList({conversations, active, friends, typingConversationHash, onSelect, onCreateGroup}: Props) {
    ...
    const selectedFriends = friends.filter(friend => selected.includes(friend.id));
    ...
    return <div className="flex h-full w-full flex-col">
        ...
        {creating && <form onSubmit={submit} className="relative space-y-3 border-b border-slate-200 p-3 dark:border-slate-800">
            ...
            <button type="button" onClick={() => setMembersOpen(v => !v)} className="flex h-10 w-full items-center justify-between rounded-md border border-slate-300 bg-white px-3 text-left text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
                ...
                <span className="text-xs text-slate-400">Required: 2+</span>
            </button>
            {selectedFriends.length > 0 && <div className="flex flex-wrap gap-1.5">{selectedFriends.map(friend => <span key={friend.id} className="rounded-full bg-purple-50 px-2 py-1 text-xs font-semibold text-purple-800 dark:bg-purple-500/15 dark:text-purple-100">{friend.name}</span>)}</div>}
            {membersOpen && <div className="absolute left-3 right-3 top-[6.75rem] z-30 max-h-64 overflow-y-auto rounded-lg border border-slate-200 bg-white p-2 shadow-2xl dark:border-slate-700 dark:bg-slate-800">
                {friends.length === 0 && <p className="px-2 py-6 text-center text-sm text-slate-500 dark:text-slate-400">No friends available.</p>}
